...
async def run_pipeline(
    ...
    result = await orchestrator.run(current_user.id, db)
    return result
...
async def get_pipeline_jobs(
    ...
    return await orchestrator.get_user_jobs(current_user.id, db)
...
async def predict(
    ...
    return {"topic": data.topic, "views": views, "revenue": revenue}
...
async def get_memory(
    ...
    return await memory_store.get_preferences(current_user.id, db)
